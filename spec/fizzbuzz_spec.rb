...
describe 'exercise cases' do
  inputs = (1..20).to_a
  outputs = [1, 2, 'fizz', 4, 'buzz', 'fizz', 7, 8, 'fizz', 'buzz', 11, 'fizz', 13, 14, 'fizzbuzz', 16, 17, 'fizz', 19, 'buzz']
  inputs.each do |index|
    it "The input #{index} should output #{outputs[index-1]}" do
      expect(fizzbuzz(index)).to eq outputs[index-1]
    end
  end
end
